  ...
  return (
    ...
    >
      {label && labelPosition === "above" ? (
        <div className={cn(LABEL_OVERLAP_ABOVE, "w-full shrink-0 max-md:hidden", LABEL_INSET_X)}>
          {label}
        </div>
      ) : null}
      ...
      {label && labelPosition === "below" ? (
        <div className={cn(LABEL_OVERLAP_BELOW, "w-full shrink-0 max-md:hidden", LABEL_INSET_X)}>
          {label}
        </div>
      ) : null}
    </figure>
