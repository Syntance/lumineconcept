...
/**
 * POST /admin/setup/payment — jednorazowy bootstrap płatności testowej.
 * Podłącza `pp_system_default` do każdego regionu, który go nie ma.
 * Wymaga nagłówka zgodnego z LUMINE_SETUP_SECRET.
 */
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  ...
  if (!expected || expected.length < 8) {
    return res.status(503).json({
      message:
        "LUMINE_SETUP_SECRET nie jest ustawiony lub jest za krótki (min. 8 znaków). Użyj lokalnie: pnpm --filter @lumine/backend setup-payment",
    });
  }
