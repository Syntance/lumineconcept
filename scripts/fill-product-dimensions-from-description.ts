/**
 * Uzupełnia `metadata.wymiary` dla każdego produktu w Medusie na podstawie
 * opisu (HTML) i opcjonalnie `metadata.specyfikacja` — ta sama logika co
 * `extractDimensionsFromProductDescription` w storefront.
 *
 * Użycie:
 *   MEDUSA_BACKEND_URL=http://localhost:9000 \
 *   MEDUSA_ADMIN_EMAIL=... MEDUSA_ADMIN_PASSWORD=... \
 *   pnpm fill-product-dimensions
 *
 * Opcje (env):
 *   DRY_RUN=1           — tylko log, bez zapisu
 *   FORCE=1             — nadpisz istniejące `metadata.wymiary`
 *   SKIP_IF_SET=0       — domyślnie 1: pomiń produkty, które już mają `wymiary`
 */
...
/** Opcjonalny plik z sekretami (lokalny, bez commita): scripts/fill-product-dimensions.env */
...
async function run() {
  ...
  console.log(
    `Tryb: ${DRY_RUN ? "DRY RUN (bez zapisu)" : "ZAPIS"} | FORCE=${FORCE} | SKIP_IF_SET=${SKIP_IF_SET}\n`,
  );
  ...
  while (true) {
    ...
    for (const row of data.products) {
      ...
      const existing =
        typeof meta.wymiary === "string" ? meta.wymiary.trim() : "";

      if (SKIP_IF_SET && existing && !FORCE) {
        console.log(`[SKIP ma wymiary] ${p.title} (${p.handle}) — "${existing}"`);
        skipped++;
        continue;
      }
      ...
      if (existing === extracted && !FORCE) {
        console.log(`[OK bez zmian] ${p.title} — ${extracted}`);
        skipped++;
        continue;
      }
